...
def _build_parser() -> argparse.ArgumentParser:
    ...
    qa_cmd = sub.add_parser("qa", help="Audit an issue against the editorial guidelines")
    qa_cmd.add_argument("week", nargs="?", help="ISO week, e.g. 2026-W32. Defaults to latest.")
    check_cmd = sub.add_parser(
        "check", help="Deterministic guideline compliance check of an issue",
    )
    check_cmd.add_argument("week", nargs="?", help="ISO week, e.g. 2026-W32. Defaults to latest.")
...
def cli() -> None:
    ...
    if args.command == "qa":
        from oykos.pipeline.runner import print_qa  # noqa: PLC0415

        asyncio.run(print_qa(getattr(args, "week", None)))
        sys.exit(0)

    if args.command == "check":
        from oykos.pipeline.runner import print_compliance  # noqa: PLC0415

        asyncio.run(print_compliance(getattr(args, "week", None)))
        sys.exit(0)
